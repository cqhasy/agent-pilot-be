...
export function renderTaskPlan(tasks) {
    // 已迁移到 AI 助手框中（chatTaskPanel），右侧文档区不再展示。
}
...
export function renderUserCard(user) {
    if (!user) {
        ...
        el.feishuLoginBtn.classList.remove("logged-in");
        el.feishuLoginBtn.textContent = "飞书登录";
        el.accountMenu?.classList.add("hidden");
        return;
    }
    // 侧边栏顶部账号胶囊
    el.feishuLoginBtn.classList.add("logged-in");
    el.feishuLoginBtn.innerHTML = `
    <span class="account-pill">
      <span class="avatar">${user.avatar ? `<img src="${escapeHtml(user.avatar)}" alt="avatar" />` : ""}</span>
      <span class="account-name">${escapeHtml(user.name || "飞书用户")}</span>
    </span>
  `;
    if (el.accountSummary) {
        el.accountSummary.innerHTML = `
      <div class="name">${escapeHtml(user.name || "飞书用户")}</div>
      <div>${escapeHtml(user.email || "未提供邮箱")}</div>
      <div class="meta">${escapeHtml(user.openId || "")}</div>
    `;
    }
}
...
export function renderContextBox() {
    ...
    el.contextBox.innerHTML = state.selectedTargets.map((item, idx) => {
        const preview = escapeHtml((item.originalText || "").slice(0, 40));
        return `
      <div class="context-item" data-target-id="${item.id}">
        <div class="context-title">
          <span>片段 ${idx + 1}（${preview}${item.originalText.length > 40 ? "..." : ""}）</span>
          <button type="button" class="chip-remove" data-remove-target-id="${item.id}">移除</button>
        </div>
        <div class="context-note-label">建议：</div>
        <textarea class="context-note-input" data-note-target-id="${item.id}" placeholder="输入该片段的改进建议（可选）">${escapeHtml(item.note || "")}</textarea>
      </div>
    `;
    }).join("");
}
...
export function bindContextActions(onRemove, onUpdateNote) {
    ...
    el.contextBox.querySelectorAll("[data-note-target-id]").forEach((input) => {
        input.oninput = () => onUpdateNote(input.getAttribute("data-note-target-id"), input.value);
    });
}
...
export function showRewriteProposal(range, originalText, replacementText) {
    ...
    actions.innerHTML = `<button class="accept" type="button">接受替换</button><button class="reject" type="button">取消</button>`;
    ...
    const apply = (accept) => {
        const textNode = document.createTextNode(accept ? replacementText : originalText);
