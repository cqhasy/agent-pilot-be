...
function getEditorSelectionRange() {
  ...
  if (
    !el.editor.contains(range.startContainer) ||
    !el.editor.contains(range.endContainer) ||
    range.collapsed
  ) {
    return null;
  }
...
export function buildTargetPrompt(targets) {
  const details = targets
    .map((item, idx) => {
      const note = (item.note || "").trim() || "请在保持原意的基础上优化表达。";
      return `- 选区 ${idx + 1}: ${item.originalText}\n  要求: ${note}`;
    })
    .join("\n");
  return `请根据以下选区和要求进行改写，并返回建议替换文本：\n${details}`;
}

export function applyReplacements(data, targets) {
  let replacements = Array.isArray(data?.suggestedReplacements)
    ? data.suggestedReplacements
    : [];
  if (!replacements.length && data?.suggestedReplacement)
    replacements = [data.suggestedReplacement];
